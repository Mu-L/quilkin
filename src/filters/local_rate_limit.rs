...
//! Controls the frequency at which packets received downstream are forwarded
...
//! ```
//!
//! To configure a rate limiter, we specify the maximum rate at which the proxy
//! is allowed to forward packets. In the example above, we configured the proxy
//! to forward a maximum of 1000 packets per 500ms (2000 packets/second).
//!
...
//!   A counter over the total number of packets that have exceeded the
//!   configured maximum rate limit and have been dropped as a result.
